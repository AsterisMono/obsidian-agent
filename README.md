...
4. Build one plugin with `pnpm --filter <package-name> build`, or build all plugins with `pnpm build`. Use `pnpm check` for type checking and `pnpm test` for desktop E2E scenarios; the latter requires a display.
