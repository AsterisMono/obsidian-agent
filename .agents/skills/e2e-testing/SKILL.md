...
- Mount only inputs the test explicitly needs: the exact read-only Nix runtime closure of the pinned Obsidian executable, one writable temporary test root containing its vault and profile, and the required display socket. Create private `/dev` and `/proc` mounts; isolate PID and IPC namespaces. Keep all XDG directories, `HOME`, and `TMPDIR` inside the temporary root.
...
- Probe the sandbox before launch: repository and host system paths such as `AGENTS.md`, `/home`, and `/etc/passwd` must be absent, while the explicitly mounted Obsidian binary, test vault, and display socket are present. Add any new filesystem dependency as a narrow, documented mount and check its necessity. Do not weaken isolation to fix a failing test.
...
- Run the plugin's UI test script from the repository root inside `devenv shell`, then run `pnpm check` and `pnpm build`. If a display is unavailable, use a Nix-provided virtual display or state that the UI test could not run.
