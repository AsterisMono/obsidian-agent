...
3. In the returned root pane, start a fresh planner using `codex` with `gpt-6-astra` and `xhigh`. Pass the original user input verbatim, all subsequent corrections, the assigned planner role, `<stem>`, descriptive title, worktree path, branch, applicable skills, and authorized endpoint. Tell it to write the design handoff below at `docs/<stem>/<stem>.md`, with supporting text-based design and proof sources beneath that folder; keep binary review artifacts outside the repository for PR attachment. Give agents stable unique coordination names such as `amber-lark-plan` and `amber-lark-impl`; progress updates change their display metadata, not these names.
...
Never commit binary files to the repository. Store rendered screenshots, mockup images, recordings, and other binary review artifacts in a task-specific temporary directory outside the checkout, and hand their paths and descriptions to the implementer for PR attachment. Keep reproducible text sources in the plan folder; use uploaded URLs for durable evidence links once available.
...
When every verification and test required for the authorized task has passed, the implementer commits in the task worktree. Review the diff and working-tree status, stage only task changes, verify that no binary files are staged, and use the repository's kernel-style `scope: msg` commit message with no body. Push `worktree/<stem>` and open a PR with base `main`. Include the concrete change, validation results, and any remaining external assumptions in the PR description. This is part of full delivery; do not ask for another confirmation to commit or open the PR.

Include the key resources a reviewer needs to assess the result. For UI changes, attach representative images from the implemented plugin, with before/after views or important states when useful; label design prototypes separately. Give each image descriptive alt text and enough context to identify its state, viewport, and theme. Link the plan, relevant text artifacts, and review evidence from the PR body. Binary resources remain outside Git, including those produced by the planner or test tooling.

Write the complete PR description to a temporary Markdown file and upload the selected images or videos with `gh pr edit --body-file ... --attach ...`, following [GitHub's attachment guide](https://docs.github.com/en/github-cli/github-cli/attaching-files-with-github-cli):

```sh
gh pr edit <pr-number-or-url> --body-file <body-file> \
  --attach '<image-file>#<descriptive-alt-text>'
```

Repeat `--attach` for multiple files. To place an image beside its explanation, reference the same local file in the Markdown body; GitHub CLI replaces that reference with the uploaded URL and preserves the Markdown alt text. Unreferenced attachments are appended. Videos use a file path without `#alt`. The CLI supports image and media uploads; do not assume it accepts arbitrary binary files or commit them as a fallback.

Read back the published body with `gh pr view <pr-number-or-url> --json body,url` and verify that the intended resources have hosted URLs. Preserve existing PR content and successful attachment URLs when making later edits. A partial upload failure can still update the PR; inspect the resulting body and retry only failed attachments with the corrected full body. If required evidence cannot be uploaded, keep attachment delivery pending and surface the blocker in the implementation session instead of reporting a complete handoff.
