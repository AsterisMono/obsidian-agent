...
Reopen these files before using the examples; the observations describe the code inspected when this skill was written. Paths below are relative to the repository root. The behavior in `docs/bright-otter-initial-plan/bright-otter-initial-plan.md` supplies the original intent; use the current written plan for subsequent work.
